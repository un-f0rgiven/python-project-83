...
def is_valid_url(url):
    ...
    # Используем validators для проверки валидности URL
...
def home():
    ...
    # Перенаправление на обработчик /urls
...
# Декоратор для маршрута, обрабатывающего POST-запросы
def create_check(url_id):
    # Проверка соединения с базой данных
    if not get_db_connection():
        flash('Не удалось подключиться к базе данных. '
              'Попробуйте позже.', 'danger')
        # Сообщение об ошибке
        return redirect(url_for('list_urls'))
        # Перенаправление на страницу списка URL
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            with conn.cursor() as cursor:
                ...
                if url_data is None:
                    ...
                    # Сообщение об ошибке, если URL не найден
                    return redirect(url_for('list_urls'))
                    # Перенаправление на страницу списка URL

                url_name = url_data[0]
                # Извлекаем имя URL из результата запроса
        ...
        with get_db_connection() as conn:
            ...
            with conn.cursor() as cursor:
                # Создаем курсор для выполнения запросов
                # Запрос для вставки данных о проверке в таблицу url_checks
                cursor.execute(
        ...
        # Сообщение об успешном завершении проверки
    ...
    )  # Перенаправление на страницу URL после завершения
